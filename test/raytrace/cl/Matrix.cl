/*
  Inspired by https://github.com/GPUOpen-LibrariesAndSDKs/RadeonProRender-Baikal/blob/master/Baikal/Kernels/CL/utils.cl
*/
...
float4 transform_vector4(matrix4x4 m, float4 v)
{
    float4 res;
    res.x = dot(m.m0, v);
    res.y = dot(m.m1, v);
...
{
    float4 d = normalize(lookat - position);
    float4 r = normalize(cross(d, up));
    float4 u = cross(r, d);
